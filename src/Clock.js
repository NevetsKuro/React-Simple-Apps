...
class Clock extends React.Component {
    ...
    getTimeUtil(date) {
        ...
        const secs = Math.abs(parseInt(Math.floor((time / 1000) % 60)));
        const mins = Math.abs(parseInt(Math.floor((time / 1000 / 60) % 60)));
        const hrs = Math.abs(parseInt(Math.floor((time / (1000 * 60 * 60)) % 24)));
        const days = Math.abs(parseInt(Math.floor((time / (1000 * 60 * 60 * 24)))));
